namespace _3.MinesweeperGame.Core
{
    ...
    public class GameDatabase : IGameDatabase
    {
        ...
        public char[,] PublicGameBoard => this.publicGameBoard;

        public char[,] PrivateGameBoard => this.privateGameBoard;

        public IList<Player> TopPlayers => this.topPlayers;
        ...
        public string RenderScoreBoard()
        {
           ...
           result.AppendLine(Environment.NewLine + "+------Top players------+");
            if (this.topPlayers.Count > 0)
            {
                ...
                {
                    result.AppendLine(
                            $"{index + 1}. ({this.topPlayers[index].Points:00}) {this.topPlayers[index].Name.ToUpper()}");
                }
            }
            else
            {
        ...
        private IList<Player> SortHighScores()
        {
